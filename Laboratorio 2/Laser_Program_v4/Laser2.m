%I programmi implementati per il modello di un laser con 2 popolazioni 
%permettono di fare l'analisi statica e dinamica delle equazioni di bilancio.
%-------------------------------------------------------------------------------------------------------%
function varargout = Laser2(varargin)
%LASER2 M-file for Laser2.fig
%Funzione per la gestione dell'interfaccia di inizializzazione
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                  'gui_Singleton',  gui_Singleton, ...
                  'gui_OpeningFcn', @Laser2_OpeningFcn, ...
                  'gui_OutputFcn',  @Laser2_OutputFcn, ...
                  'gui_LayoutFcn',  [] , ...
                  'gui_Callback',   []);
...
%-------------------------------------------------------------------------------------------------------%
function Laser2_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Laser2
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
%-------------------------------------------------------------------------------------------------------%
function varargout = Laser2_OutputFcn(hObject, eventdata, handles)
...
prompt  = {'Wavelenght of Laser2 [nm]'};
dlgTitle   = 'Wavelenght';
lineNo = 1;
def1     = {'1550'};
...
%Lambda d'operazione  
handles.Lambda=str2double(char(handles.answer1(1)))*1e-7;%lung d'onda [cm]
%Costanti universali
...
lineNo=1;
def2={'10','2.5','900','0','76','0.06','0','[1e-4]','[0.3]','[0.9]','4.2'};
...
handles.WscH=str2double(char(handles.answer2(5)))*1e-7;   % Larghezza cavita SCH espressi in [cm]
...
prompt  = {'Differential Gain [cm^2] --> a0',...
           ...
           '\tau_e [ns] --> Escape time from the well',...
           '\tau_s [ns] --> Capture time from the well',...
           '\tau_B [ns] --> Recombination time in the barrier'};
dlgTitle   = 'Materials Parameters';
lineNo = 1;
def3     = {'[8.33e-14]','0.8','1.8e18','[1.4e-15]','0e-32','3.57e8',...
            '0.8e-10','3.5e-30','[6]','[0]','[1]','5','60e-3','60e-3'};
...
handles.Taue=str2double(char(handles.answer3(12)))*1e-9;% Tempo di fuga dal well [s]
handles.Taus=str2double(char(handles.answer3(13)))*1e-9;% Tempo di cattura dal well [s]
handles.Taunrb=str2double(char(handles.answer3(14)))*1e-9;% Tempo di ricombinazione nella barriera [s]
guidata(gcbo,handles);
...
%Generate Pout linearly spaced vectors
...
handles.VscH=handles.d*handles.WscH*handles.La;  %Volume della cavità SCH [cm^3]
...
handles.Gamq=handles.V/handles.VscH;% Gama nella sessione SCH
...
handles.Tausap = handles.Taus*handles.Taunrb/...
                (handles.Taus+handles.Taunrb)*2;% Tempo decadimento portatori nella barriera
...
handles.Tau=[]; handles.Nb=[]; handles.I=[]; handles.N=[]; handles.Np=[];
...
if strcmp(vettorescansione,'')==0 && strcmp(handles.quantitascansione,'')==0 
    ...
    for j=1:1:BB1
        ...
        handles.Np=handles.Pout*handles.Taup./...
        ...
        for i=1:1:cicli
            ...
            if(NumNth>1)
                  ...
                  handles.N(i)=fzero(handles.f,[0 handles.Nth*20]);
            end
            handles.Tau(i)=(handles.A+handles.B*handles.N(i)+...
                            handles.C*handles.N(i).^2);
            handles.Nb(i)=handles.Gamq*handles.Taus*(handles.N(i)*(1/handles.Tau(i)+...
                          1/handles.Taue)+handles.Np(i)/(handles.Taup*handles.Gam)-...
                          handles.B1*handles.B*handles.N(i)^2);
            handles.I(i)=handles.qv/(handles.Gamq*handles.ni)*(handles.Nb(i)/...
                         handles.Tausap+handles.Gamq*handles.N(i)/handles.Taue);
            %if(NumNth>1)
...
handles.dnb = 0;  % Variazioni indice portatori barriera
%Chiamata alla finestra di prompt e impostazione dei suoi parametri
...
handles.Nth=[]; handles.Ith=[];
[Ith]=GetSoglia(handles.I,handles.Pout);
handles.Ith=Ith;
handles.Nth=handles.Ntr+handles.gth/handles.a0;
if strcmp(defaultvalue,'none')==0
...
switch get(handles.TypeDynamic,'Value')
   ...
   case 3  %Analisi Switching On
       ...
       handles.title=2;
       guidata(gcbo,handles);
       set(handles.RunD2,'Visible','on'); 
       set(handles.RunD2,'Position',get(handles.RunA,'Position'));
...
handles.t22=text(handles.Imaxcs*1e3,handles.N(100)/2,...
                'Imax','HorizontalAlignment','left','FontSize',10);
subplot(3,1,3);
handles.s31=stem(handles.Iocs*1e3,handles.Nb(100),'fill',':.r','LineWidth',2);
handles.t31=text(handles.Iocs*1e3,handles.Nb(100)/2,'Imin',...
                'HorizontalAlignment','right','FontSize',10);
handles.s32=stem(handles.Imaxcs*1e3,handles.Nb(100),'fill',':.b','LineWidth',2);
handles.t32=text(handles.Imaxcs*1e3,handles.Nb(100)/2,...
                'Imax','HorizontalAlignment','left','FontSize',10);
...
%Valori iniziali
vin=[0.5e18; 0.5e18; 0.5e18];
waitbar(2/4);
%integrates the system of differential equations from time t0 to tf with
%initial conditions y0.
options=odeset('initialstep',1e-3, 'AbsTol',1e-6, 'RelTol', 1e-3,'MaxStep',0.1e-10);
[t,y]=ode15s(@Ele,T, vin,options,handles,handles.title); %y =[dNb;dN;dNp]
Nb=y(:,1);                    %Densita dei portatori nelle barriere
N=y(:,2);                     %Carrier density
Np=y(:,3);                    %Photon density
waitbar(3/4);
[I]=Curre_Sig(t,handles,handles.title);   %The current signal
Av=(handles.Alf_H.*handles.Vg.*handles.Gam*handles.a0.*...
   (N-handles.Nth))./(4*pi)+(2*pi*(handles.v/handles.ng)*...
    handles.Gamb*handles.dnb*Nb);  %Shift frequency
Pout=(handles.n0*handles.h*handles.v*handles.Vp/...
     (handles.Taup)).*Np;  %output power
waitbar(4/4);
close(h);
%Esecuzione grafici
GraficiD(t,I,Pout,N,Av,Nb,handles);
%Valori per analisi AM e FM
...
Av=(handles.Alf_H.*handles.Vg.*handles.Gam*handles.a0.*...
   (N-handles.Nth))./(4*pi)+(2*pi*(handles.v/handles.ng)*...
    handles.Gamb*handles.dnb*Nb); %Shift frequency
Pout=(handles.n0*handles.h*handles.v*handles.Vp/...
     (handles.Taup)).*Np;  %Potenza di uscita
waitbar(4/4);
close(h);
%Esecuzione grafici
GraficiD(t,I,Pout,N,Av,Nb,handles);
...
if AN=='AM'
    ...
    waitbar(5/6);
    %Grafico
    figure('Name','AM analysis','NumberTitle','off');
...
elseif AN=='FM'
    ...
    waitbar(5/6);
    %Grafici
    figure('Name','FM analysis','NumberTitle','off');
...
Av=(handles.Alf_H.*handles.Vg.*handles.Gam*handles.a0*...
   (N-handles.Nth))./(4*pi)+(2*pi*(handles.v/handles.ng)*...
    handles.Gamb*handles.dnb*Nb);  %Frequency Shitf
Pout=(handles.n0*handles.h*handles.v*handles.Vp/...
     (handles.Taup)).*Np;  %output power
...
set(handles.R_CER,'Visible','on'); 
set(handles.R_CER,'String',num2str(handles.CER_dB,'%.2f'));
set(get(hObject,'parent'),'Pointer','arrow');
%-------------------------------------------------------------------------------------------------------%
...
%N,Nb e Np contengono i valori dei vettori di stato
handles.Nb = invet(1);  
handles.Nn = invet(2);
handles.Np = invet(3);                    
...
%Rate equations
handles.dNb=handles.Gamq*handles.ni.*I./(handles.q*handles.V)-...
            handles.Nb./handles.Tausap+handles.Gamq.*handles.Nn./...
            handles.Taue;%Densita dei portatori nelle barrire / t
handles.dN=handles.Nb./(handles.Gamq*handles.Taus)-handles.Nn.*...
           (1/handles.Tau+1/handles.Taue)-handles.Vg.*handles.Np*...
           handles.go./(1+handles.Ep.*handles.Np+handles.Ep2*...
           handles.Np^2);  %Densita dei portatori / t
handles.dNp=handles.Np.*(handles.Gam*handles.Vg*handles.go./...
            (1+handles.Ep.*handles.Np+handles.Ep2.*handles.Np^2)-...
            1/handles.Taup)+handles.Espont;  %Densita dei fotoni / t
out=[handles.dNb; handles.dN; handles.dNp];
guidata(gcbo,handles);
%-------------------------------------------------------------------------------------------------------%
%Simula il comportamento del segnale di corrente iniettata nel laser 
function [I]=Curre_Sig(t,handles,tipo1)
switch(tipo1)
    case 1%Electro - Optic
        %Step di corrente
        I = handles.Iocs*(t>=handles.Tstart-2e-9)+handles.Imax*(t>=handles.Tst);
        guidata(gcbo,handles);
    case 2%Switching On
        %Gradino
        I = handles.Iocs*(t>=handles.Tstart-2e-9)+(handles.Imaxcs-handles.Iocs)*(t>=handles.Tst);  
        guidata(gcbo,handles);  
    case 3%PRBS
        Tb = 1/handles.Rb; % Tempo di bit
        Ipico = handles.Imaxcs-handles.Iocs;                %Ampiezza picco picco
        vett_PRBS = prbs(handles.Nbit);
        I=zeros(1,length(t));
        for k=1:length(t)
            I(k) = handles.Iocs*(t(k)>=handles.Tstart)+pulse_PRBS(t(k),vett_PRBS,Tb,4,Ipico)*(t(k)>=handles.Tst);
        end
        guidata(gcbo,handles);  
end
%-------------------------------------------------------------------------------------------------------%
...
gradeplots;
subplot(3,1,3);
plot(handles.I*1e3,handles.Nb,'LineWidth',2);
hold on;
leg{i}=[handles.quantitascansione ' = ',num2str(handles.ValorePerlegenda)];
legend(leg,4);
grid on;
xlabel('Current [mA]');
ylabel('Carrier density in the Barrier [cm-3]');
title('Carrier density in Barrier vs I');
gradeplots;
guidata(gcbo,handles);
%-------------------------------------------------------------------------%
function GraficiD(Gt,GI,GPout,GN,GAv,GNb,handles)
...
set(ax(2),'YAxisLocation','l','Ycol','r','YMinorGrid','on');
subplot(2,2,3,'v6');
[ax,h1,h2] = plotyy(Gtp1*1e9,GIp1*1e3,Gtp1*1e9,GNbp1,'plot');
grid on;
set(get(ax(1),'Ylabel'),'String','Current [mA]');
set(get(ax(2),'Ylabel'),'String','Carrier Density in the barrier [cm^-3]');
xlabel('Time [ns]');
set(ax,'xlim',[Gxmin Gxmax]);
title('Carrier Density in the barrier - Current vs Time');
set(h1,'LineStyle',':','color','b','LineWidth',2.5);
set(h2,'LineStyle','-','color','r','LineWidth',2.5);
set(ax(1),'YAxisLocation','r','Ycol','b');
set(ax(2),'YAxisLocation','l','Ycol','r','YMinorGrid','on');
