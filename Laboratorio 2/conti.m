...
r_2=0.9;
La=900e-4;
alpha_m = (1/La)*(log(1/(sqrt(r_1*r_2)))); % rircordati che è cmetri^-1
alpha_i = 6; %unità di misura cmetri^-1
...
alpha_m = (1./La).*(log(1/(sqrt(r_1*r_2)))); % rircordati che è cmetri^-1
alpha_i = 3; %unità di misura cmetri^-1
gamma_per_g_threshold=alpha_m+alpha_i;
N_tr=1.8e18;%elettroni per centrimetro cubo
a_0 =  1e-16; %sono cm^2 
guadagno_modale = a_0 .* gamma_per_g_threshold;
